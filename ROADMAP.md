...
### Active Follow-Ups
- The five tier1 memory-model freeze decisions (#326-#330) are **resolved** on master (see Recently Shipped, July 2026); #331 (hot-getter fast path) also shipped with the perf batch. The tier2/tier3 memory-model follow-through has now largely shipped (see the grid render/memory sprint above): #332 (GridData SoA), #334 (DiscreteMap buffer protocol), #335 (ColorLayer/TileLayer numpy views) closed; #338 shipped as a safe subset (issue left open for the deferred `render_sprite` relocation). Still open: **#333** (lazy TCOD map rebuild, unblocked by #332), **#352** (perspective-grid render early-out), and **#336** (free-threading hardening).
...
- 0.2.8 is released to master (`cf844f4`, pushed) but the `0.2.8` git tag is still local-only -- push it when ready to publish the release. The full July batch (perf, tier1 decisions, Gauntlet, grid render/memory sprint) is now pushed to origin/master (tip `f68a770`); the `closes #...` commit trailers took their issues down on merge.
