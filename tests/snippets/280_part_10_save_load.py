# mcrf: objects=[Caption,Color,ColorLayer,Entity,FOV,Frame,Grid,InputState,Key,Scene,Texture] verified=0.2.8-dev status=ok
...
def try_move_or_attack(dx: int, dy: int) -> None:
    ...
    if blocker is not None:
        ...
        player.grid_pos = (new_target_x, new_target_y)
...
def move_toward_player(enemy: mcrfpy.Entity, ex: int, ey: int, px: int, py: int) -> None:
    ...
    if can_move_to(grid, new_x, new_y, enemy):
        ...
        enemy.grid_pos = (ex + dx, ey)
    elif dy != 0 and can_move_to(grid, ex, ey + dy, enemy):
        enemy.grid_pos = (ex, ey + dy)
