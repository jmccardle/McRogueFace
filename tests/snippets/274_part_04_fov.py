# mcrf: objects=[Caption,Color,ColorLayer,Entity,FOV,Grid,InputState,Key,Scene,Texture] verified=0.2.8-dev status=ok
...
def update_fov(
    ...
    Args:
        ...
        radius: How far the player can see (defaults to FOV_RADIUS)
    """
    # Compute FOV from player position
    grid.compute_fov((player_x, player_y), radius=radius, algorithm=mcrfpy.FOV.SHADOW)
...
pos_display = mcrfpy.Caption(
    pos=(50, 660),
    text=f"Position: ({player.grid_x}, {player.grid_y})"
)
